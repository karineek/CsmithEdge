...
if [ "$#" -ne 4 ]; then
	echo "Illegal number of parameters. Please enter the base folder and 0 or 1 (csmithedge or lazy)."
	exit 1
fi

## Check the folder exists
if [ ! -d $base/scripts/CsmithEdge ] ; then
	echo "Cannot find the base folder. No such path: <$base/scripts/CsmithEdge>"
	exit
fi
