...
function diff_testing {
	...
	do
		## Generate
		$AE_path/../scripts/diff_testing_rate/WA1_post_gen_test.sh $seed $base $timeout_bound >> $output 2>&1
	...
	echo " >> CsmithEdge Generated ($Y) Programs in ($T) time; it got ($X) timed-out or UB programs ($ZT of which are timed-out), and ($Z) valid and terminating within $timeout_bound s programs."
}
################################################# MAIN #################################################
base=$1
seeds=$2
