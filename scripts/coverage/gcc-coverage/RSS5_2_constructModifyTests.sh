...
## Code to check MIX for csmith too
function replace2AllsafeMix {
	testcaseName=$1
...
function modify_test {
	...
	ulimit -St 150;$csmith_exec --seed $seed $CSMITH_USER_OPTIONS > $testcaseModify  
	if [[ "$modify" == "1" ]]; then	
		## WA + RRS ##
		...
		# Compile the modify program: (with input compiler flags or with defualts)
		#echo "ulimit -St 500;$compiler $compile_line $compile_line_confg ${compilerflags} $testcaseModify -o $testcaseEXEC"
		ulimit -St 300;$compiler $compile_line $compile_line_confg ${compilerflags} $testcaseModify -o $testcaseEXEC
	elif [[ "$modify" == "2" ]]; then ## MIX-Csmith, WA-MIX
		## MIX from configuration file
		#echo "ulimit -St 500;$compiler $compile_line $compile_line_confg ${compilerflags} $testcaseModify -o $testcaseEXEC"
		if [ $headerMode -eq 2 ] && [ ${#inocationsMacrosMix[@]} -gt 0 ]; then
			replace2AllsafeMix $testcaseName
		fi
		ulimit -St 300;$compiler $compile_line $compile_line_confg ${compilerflags} $testcaseModify -o $testcaseEXEC
	else
		## WA only MACROS or FUNCTIONS ##
		# Compile the modify program WA only (with input compiler flags or with defualts)
		## Override configuaration file with value from ${runtime_default} 
		#echo "ulimit -St 500;$compiler $compile_line ${runtime_default} ${compilerflags} $testcaseModify -o $testcaseEXEC"
		ulimit -St 300;$compiler $compile_line ${runtime_default} ${compilerflags} $testcaseModify -o $testcaseEXEC
 	fi
	
	# test the modify program: (with input compiler flags or with defualts)
	
	# Test exec	
	filesize=`stat --printf="%s" $testcaseModify`
	ulimit -St 50;./$testcaseEXEC >> $testcaselogger
...
function coverage_test_only {
	...
	# Modify the test (the preprocessed file)
	ulimit -St 150;$csmith_exec --seed $seed $CSMITH_USER_OPTIONS > $testcaseOriginal
	...
	ulimit -St 300;$compiler $compile_line ${runtime_default} ${compilerflags} $testcaseOriginal -o $testcaseEXEC
...
runtime_default=${14}		# Runtime Csmith defualt lib (with no RRS)
...
#echo "runtime_default=$runtime_default"
